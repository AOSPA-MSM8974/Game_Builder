...
android {
    ...
    // Tell Android where the unpacked .so files live
    sourceSets {
        getByName("main") {
...
dependencies {
    ...
    // x86_64 MUST be before x86 to avoid substring mismatch
...
// Configuration-cache-safe task using injected FileSystemOperations + ArchiveOperations
abstract class CopyAndroidNativesTask @Inject constructor(
    ...
    @TaskAction
    fun run() {
        // x86_64 MUST come before x86 to avoid substring match on the wrong ABI
...
// Auto-runs before every build — no manual step needed
tasks.configureEach {
